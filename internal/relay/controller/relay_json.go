...
func (rc *RelayController) relayJSON(c *gin.Context, requestID string, startTime time.Time, mode constant.RelayMode, format constant.RelayFormat, meta relayRequestMeta, body []byte, candidates []relayCandidate) {
	var lastErr error
	var lastErrMsg string
	attemptedUpstream := false

	for _, candidate := range candidates {
		info := buildRelayInfo(c, requestID, startTime, mode, format, meta, candidate, false)
