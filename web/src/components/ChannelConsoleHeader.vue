...
<template>
  <header class="channel-queue-header">
    ...
    <div class="queue-status-grid" aria-label="渠道状态筛选">
      <button
        type="button"
...
.queue-head-row { display: flex; min-width: 0; flex-wrap: wrap; align-items: flex-end; justify-content: space-between; gap: 12px; }
.queue-head-tools { display: flex; min-width: 0; flex: 1 1 20rem; align-items: center; justify-content: flex-end; gap: 8px; }
.queue-search { position: relative; display: block; min-width: 0; flex: 0 1 22rem; }
.queue-status-grid { display: grid; grid-template-columns: repeat(5, minmax(0, 1fr)); gap: 6px; }
.queue-status-button,
.route-bus-segment {
...
@media (max-width: 900px) {
  .queue-status-grid { grid-template-columns: repeat(3, minmax(0, 1fr)); }
}
@media (max-width: 560px) {
  .queue-head-tools { justify-content: flex-start; }
  .queue-search { flex-basis: 100%; }
}
