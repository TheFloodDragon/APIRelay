...
- [x] 阶段4：模型聚合增强（优先级分层+加权随机）+ 故障转移精细化（同渠道重试+冷却）
- [x] 阶段5：额度预扣/结算 + 异步用量 + 管理后台鉴权
- [x] 阶段6：Vue3 前端（embed，亮/暗双主题）

### 最近更新 (v0.5.0)

- ✅ **阶段5完成** - 计费体系：按模型价格的额度预扣 → 实际用量结算 → 失败退款
- ✅ **异步用量** - 调用日志落库与配额结算走后台 worker，不阻塞转发主路径
- ✅ **价格配置** - 全局价格表 + 供应商/模型级价格覆盖（USD / 1M tokens）
- ✅ **令牌额度** - 货币额度（微美元）原子预扣，防并发超额；不限额令牌仅统计用量
- ✅ **阶段6完成** - 全站 Vue3 重设计 + 亮/暗双主题 + 模型聚合页 + 价格/协议设置页
